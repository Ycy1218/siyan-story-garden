...
const initialStore = { selectedId: "qiuyu", filter: "all", outlines: {}, drafts: {}, records: [] };
let store = loadStore();
...
function getDraft(id = store.selectedId) {
  if (!store.drafts[id] || typeof store.drafts[id] !== "object" || Array.isArray(store.drafts[id])) store.drafts[id] = { paragraphOne: "", paragraphTwo: "", checks: Array(checks.length).fill(false) };
...
function renderMaterials() {
  ...
  materials.innerHTML = `
    <section class="materials-intro">
      <p class="section-kicker">三份学习资料，已经整理在这里</p>
      <h2>从看懂故事，到写出自己的两段</h2>
      <p>先把故事写顺、句子写对，再慢慢加上动作、情绪和细节。</p>
    </section>
    <section class="materials-grid">
      <article class="material-panel terms-panel"><p class="section-kicker">先认识题型</p><h2>四个词，先用大白话懂</h2><dl class="term-list">${foundationTerms.map(([term, explanation]) => `<div><dt>${term}</dt><dd>${explanation}</dd></div>`).join("")}</dl></article>
      <article class="material-panel method-map"><p class="section-kicker">写前一分钟</p><h2>四步倒推法</h2><ol class="method-list">${[["找问题", "原文最后，人物最担心什么？"], ["看段首", "第一段必须从哪里开始？"], ["倒推桥梁", "第一段写到哪里才能接住第二段？"], ["定结局", "问题如何合理解决，人物有什么成长？"]].map(([title, description]) => `<li><b>${title}</b><span>${description}</span></li>`).join("")}</ol></article>
      <article class="material-panel story-panel"><p class="section-kicker">短故事示范</p><h2>先用中文走完情节</h2><div class="story-list">${beginnerStories.map(([title, chain, start]) => `<div><b>${title}</b><span>${chain}</span><em>${escapeHtml(start)}</em></div>`).join("")}</div></article>
    </section>
    <section class="roadmap-section"><div class="section-heading"><div><p class="section-kicker">第一个月</p><h2>四周入门路线</h2></div><p>每周只升级一点点，不急着一开始写 150 词。</p></div><div class="roadmap-grid">${monthRoadmap.map(([week, title, copy], index) => `<article class="roadmap-step"><span>${String(index + 1).padStart(2, "0")}</span><p>${week}</p><h3>${title}</h3><small>${copy}</small></article>`).join("")}</div></section>
    <section class="toolkit-section"><div class="section-heading"><div><p class="section-kicker">句子工具箱</p><h2>8 句基础骨架</h2></div><p>先读懂，再替换人物、地点或动作。</p></div><div class="sentence-grid">${sentenceToolkit.map(([tag, sentence, meaning]) => `<article class="sentence-card"><span>${tag}</span><strong>${escapeHtml(sentence)}</strong><p>${meaning}</p></article>`).join("")}</div></section>
    <section class="exam-section"><div class="section-heading"><div><p class="section-kicker">真题资料</p><h2>四道题，按难度练</h2></div><p>每道题都有段首、情节链、8 句骨架和五节点构思。</p></div><div class="exam-grid">${exercises.map((item, index) => `<article class="exam-card"><span>${String(index + 1).padStart(2, "0")}</span><p class="tag ${item.level === "进阶" ? "advanced" : ""}">${item.level}</p><h3>${escapeHtml(item.title)}</h3><small>${escapeHtml(item.source)}</small><p>${escapeHtml(item.chain)}</p><button class="text-button" type="button" data-action="open-material-exercise" data-id="${item.id}">查看拆解 →</button></article>`).join("")}</div></section>
    <section class="materials-footer"><div><p class="section-kicker">交卷前</p><h2>8 项检查清单</h2></div><ul>${checks.map((item) => `<li>${item}</li>`).join("")}</ul></section>`;
}
  ...
  document.getElementById("writing-workspace").innerHTML = `
    <div class="writing-layout">
      <aside class="writing-brief"><p class="section-kicker">当前题目</p><h2>先看约束，再动笔</h2><select data-action="choose-writing-exercise" aria-label="选择练习题目">${exercises.map((item) => `<option value="${item.id}" ${item.id === exercise.id ? "selected" : ""}>${escapeHtml(item.title)} · ${item.level}</option>`).join("")}</select>
        ...
        <details><summary>展开 8 句基础骨架</summary><ol class="skeleton-sentences">${exercise.sentences.map((sentence) => `<li>${escapeHtml(sentence)}</li>`).join("")}</ol></details>
      </aside>
      <section class="writing-area"><div class="editor-head"><h2>写你的两段</h2><span class="word-meter" id="word-count">${count} 词</span></div><label for="paragraph-one">第一段：接住第一个段首，写动作、变化，并铺垫第二段。</label><textarea id="paragraph-one" data-draft-part="paragraphOne" placeholder="在这里写第一段……">${escapeHtml(draft.paragraphOne)}</textarea><label for="paragraph-two">第二段：接住第二个段首，解决问题，写出结果和感悟。</label><textarea id="paragraph-two" data-draft-part="paragraphTwo" placeholder="在这里写第二段……">${escapeHtml(draft.paragraphTwo)}</textarea><div class="write-actions"><p class="save-note">内容会自动保存在当前浏览器。</p><div class="action-row"><button class="button danger" type="button" data-action="clear-writing">清空草稿</button><button class="button primary" type="button" data-action="save-review">保存到复盘</button></div></div></section>
    </div>
    <section class="check-panel"><p class="section-kicker">交卷前检查</p><h2>用一分钟做这 8 项检查</h2><div class="check-list">${checks.map((item, index) => `<label class="check-item"><input type="checkbox" data-check-index="${index}" ${draft.checks[index] ? "checked" : ""}/><span>${item}</span></label>`).join("")}</div></section>`;
}
...
function renderReview() {
  ...
  document.getElementById("review-workspace").innerHTML = `
    <div class="review-summary"><section class="review-card"><p class="section-kicker">最近七天</p><h2>${getStreak() ? `${learnerName}已经连续练习 ${getStreak()} 天` : `${learnerName}，今天完成第一次练习`}</h2><p class="lede">这一周保存了 ${recordsThisWeek().length} 次练习，累计写下 ${totalWords()} 个英语词。</p><div class="week-strip">${days.map((date) => `<div class="week-day ${finishedDates.has(dayKey(date)) ? "is-complete" : ""}"><span>${["一", "二", "三", "四", "五", "六", "日"][(date.getDay() + 6) % 7]}</span><b>${finishedDates.has(dayKey(date)) ? "✓" : "·"}</b></div>`).join("")}</div></section>
      <section class="history-panel"><p class="section-kicker">下一次改什么</p><h2>每次只改一个重点</h2><p class="lede">先保证故事衔接和过去时，再增加动作、语言、心理等细节。</p></section></div>
    <section class="history-panel history-panel-spaced"><p class="section-kicker">保存记录</p><h2>${learnerName}的练习历史</h2>${recent.length ? `<div class="history-list">${recent.map((record) => `<article class="history-item"><div><h3>${escapeHtml(record.title)}</h3><p>${record.words} 词 · 已完成 ${record.checkedCount}/${checks.length} 项检查</p></div><time>${formatDate(record.savedAt)}</time></article>`).join("")}</div>` : `<div class="empty-state"><h2>${learnerName}还没有保存的练习</h2><p>完成一次两段写作后，点击“保存到复盘”，进度会出现在这里。</p><button class="button primary" type="button" data-view="write">开始写作</button></div>`}</section>`;
}

function renderAll() { renderOverview(); renderMaterials(); renderPractice(); renderOutline(); renderWriting(); renderReview(); }
...
document.addEventListener("click", (event) => {
  ...
  if (action === "save-review") {
    const exercise = getExercise(); const draft = getDraft(); const words = wordCount(`${draft.paragraphOne} ${draft.paragraphTwo}`);
    ...
    if (store.records[0]?.signature === signature) { showToast("当前草稿已经保存到复盘。 "); return; }
    store.records.unshift({ id: `${Date.now()}`, exerciseId: exercise.id, title: exercise.title, words, checkedCount: draft.checks.filter(Boolean).length, savedAt: new Date().toISOString(), signature });
    persist(); renderOverview(); renderPractice(); renderReview(); showToast("已收藏到思妍的成长记录。 "); return;
  }
...
document.addEventListener("input", (event) => {
  if (event.target.dataset.outlineKey) { getOutline()[event.target.dataset.outlineKey] = event.target.value; persist(); }
...
document.addEventListener("change", (event) => {
  if (event.target.dataset.checkIndex) { getDraft().checks[Number(event.target.dataset.checkIndex)] = event.target.checked; persist(); }
